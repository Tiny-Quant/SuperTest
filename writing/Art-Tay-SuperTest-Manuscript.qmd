...
notate the combined graph generating distribution, where $\Theta$ is the set of all parameters. 
...
where $\hat A_0 = p(\hat A_0 \ | \ A_t, t, X_0, E_0; \ \hat \Omega)$, $A_t[i, j] \sim q(A_t[i, \ j] \ | \ A_{0}[i, \ j])$, and 
$$
    \bar \beta_t = \frac 1 2 - \frac 1 2 \prod^t_{i=1}(1-2\beta_i), 
$$ 

the cumulative transition probability. After the denoising model has been trained, model-level explanations are produced by iterative sampling candidate adjacency structures for an observed set of graph features and selecting the candidate with the highest probability of being from the targeted class. This is a multi-step procedures, where at each step $A_t$ is denoised to $k$ candidates $\hat A_{0, 1 \dots k}$. The candidate with the best predicted probability is then noised to a level of $t-1$ and the process is repeated. The pseudocode is reproduced below.
\begin{algorithm}
