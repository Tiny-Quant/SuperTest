...
- Explanation of the graph generating distribution.  
    ...
    - The author assume that the true underlying distribution for every discrete node and edge feature is *categorical*. The categorical distribution is also know as the multi-bernoulli, where every sample has a fixed probability of being in one of the discrete categories. This distribution as a continuous approximation known as the Gumbel-Softmax [cite]. The standard Gumbel or extreme value distribution is equivalent to the density of the maximum order statistic of a standard normal. Adding Gumbel noise to categorical probabilities creates random samples 
