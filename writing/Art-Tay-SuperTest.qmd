...
- In-Distribution metrics. 
    ...
    - The author use MMD statistics to compare the degree, clustering, and spectrum distributions of the generated explanation against the observed data. The degree distribution of a graph shows the frequency of nodes with varying numbers of connections, providing insight into the network's connectivity pattern. The clustering coefficient measures the extent to which a node's neighbors are interconnected, representing the fraction of triangles involving that node and indicating local clustering. The spectrum distribution involves the eigenvalues of the graph's adjacency matrix or Laplacian matrix, offering detailed information about the graph's structural properties and dynamic behaviors. 
