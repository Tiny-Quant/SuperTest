...
\begin{document}
\maketitle
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[frame hidden, borderline west={3pt}{0pt}{shadecolor}, interior hidden, breakable, sharp corners, boxrule=0pt, enhanced]}{\end{tcolorbox}}\fi
...
\item
  ...
  \item
    ...
    mean embeddings: \begin{align*}
       MMD(\vec x, \vec y) 
          ...
          &= 
          (\dfrac{1}{n} \phi(\vec{x}) \mathbb{1} -
          \dfrac{1}{m} \phi(\vec{y}) \mathbb{1})^T
          (\dfrac{1}{n} \phi(\vec{x}) \mathbb{1} -
          \dfrac{1}{m} \phi(\vec{y}) \mathbb{1})^T
          ||^2_2 \\ 
          &= 
  ...
  \item
    ...
    observed data. The degree distribution of a graph shows the
    frequency of nodes with varying numbers of connections, providing
    insight into the network's connectivity pattern. The clustering
    coefficient measures the extent to which a node's neighbors are
    interconnected, representing the fraction of triangles involving
    that node and indicating local clustering. The spectrum distribution
    involves the eigenvalues of the graph's adjacency matrix or
    Laplacian matrix, offering detailed information about the graph's
    structural properties and dynamic behaviors.
  \end{itemize}
\item
  Loss CF
  ...
  \tightlist
  \item
    In additional to model-level explanations,
  \end{itemize}
\item
  CF-ACC
\item
